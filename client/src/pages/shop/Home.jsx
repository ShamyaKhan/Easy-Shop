...
function ShopHome() {
  ...
  function handleNavigateToShopListPage(currentItem, section) {
      ...
      <div className="relative w-full h-150 overflow-hidden">
        {slides.map((slide, idx) => (
          <img
            src={slide}
            key={idx}
            className={`${idx === currentSlide ? "opacity-100" : "opacity-0"} absolute top-0 left-0 w-full h-full object-cover transition-opacity duration-1000`}
          />
        ))}
        <Button
          variant="outline"
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {categoriesWithIcon.map((item, idx) => (
              <Card
                ...
                onClick={() => handleNavigateToShopListPage(item, "category")}
              >
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {brandsWithIcon.map((item, idx) => (
              <Card
                key={idx}
                className="cursor-pointer hover:shadow-lg transition-shadow"
                onClick={() => handleNavigateToShopListPage(item, "brand")}
              >
                <CardContent className="flex flex-col items-center justify-center p-6">
                  <item.icon className="w-12 h-12 mb-4 text-primary" />
                  <span className="font-bold">{item.label}</span>
